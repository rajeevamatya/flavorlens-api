...
async def get_summary_stats(ingredient: str = Query(..., description="Ingredient name")):
    ...
    try:
        ...
        menu_previous = data_by_source_year.get(f'menu_{previous_year}', {'share_percent': 0, 'ingredient_dishes': 0})
        social_current = data_by_source_year.get(f'social_{CURRENT_YEAR}', {'share_percent': 0, 'ingredient_dishes': 0})
        ...
        total_current = recipe_current['ingredient_dishes'] + menu_current['ingredient_dishes'] + social_current['ingredient_dishes']
        total_previous = recipe_previous['ingredient_dishes'] + menu_previous['ingredient_dishes'] + social_previous['ingredient_dishes']
        
        if total_previous > 0:
            yoy_growth = ((total_current - total_previous) / total_previous) * 100
        else:
            yoy_growth = 100 if total_current > 0 else 0

        # Determine lifecycle phase
        lifecycle_phase, phase_number = determine_lifecycle_phase(total_current, total_previous, yoy_growth)
        ...
        metrics = [
            ...
            MetricBox(
                title="Menu Share",
                value=f"{menu_current['share_percent']:.1f}%",
