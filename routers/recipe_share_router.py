# # routers/recipe_share_router.py
# from fastapi import APIRouter, HTTPException, Query
# from database.connection import execute_query, QueryOptions
# from database.models import ShareData, Share

# router = APIRouter()

# @router.get("/recipe-share", response_model=Share)
...
#             SELECT 
#                 (SELECT COUNT(DISTINCT dish_id) FROM flavorlens.main.dishes WHERE source = 'recipe') AS total_recipes,
#                 COUNT(DISTINCT d.dish_id) AS ingredient_recipes,
...
#         )
#         SELECT 
#             ROUND((id.ingredient_recipes * 100.0 / NULLIF(id.total_recipes, 0)), 1) AS recipe_share_percent,
#             (
#                 SELECT ROUND((
#                     (id.ingredient_recipes * 100.0 / NULLIF(id.total_recipes, 0)) - 
#                     ((id.ingredient_recipes - COUNT(DISTINCT d.dish_id)) * 100.0 / NULLIF(id.total_recipes, 0))
#                 ), 1)
#                 FROM flavorlens.main.dishes d
#                 JOIN flavorlens.main.dish_ingredients di ON d.dish_id = di.dish_id
#                 WHERE di.name ILIKE {ingredient_pattern}
...
#         if result["rows"] and result["rows"][0]["recipe_share_percent"] is not None:
#             row = result["rows"][0]
#             return ShareData(
#                 recipe_share_percent=float(row["recipe_share_percent"]) if row["recipe_share_percent"] else 0.0,
...
@router.get("/recipe-share", response_model=RecipeShare)
async def get_recipe_share(ingredient: str = Query(..., description="Ingredient name")):
    try:
        ...
        FROM (
            SELECT 
                ...
                COUNT(DISTINCT dish_id) FILTER (
                    WHERE ingredient_name ILIKE {ingredient_pattern} AND source = 'recipe'
        ...
        if result["rows"] and result["rows"][0]["recipe_share_percent"] is not None:
            row = result["rows"][0]
            return RecipeShare(
                share_percent=float(row["recipe_share_percent"]) if row["recipe_share_percent"] else 0.0,
                change_percent=float(row["change_percent"]) if row["change_percent"] else 0.0,
            )

        return RecipeShare(share_percent=0.0, change_percent=0.0)
