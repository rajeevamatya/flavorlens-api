...
#             category_filter = f"AND d.general_category ILIKE '{category.capitalize()}%'"
        
#         applications_query = f"""
#         WITH ingredient_applications AS (
#             SELECT DISTINCT
#                 d.dish_id,
#                 d.general_category AS category,
#                 d.specific_category AS application,
#                 d.star_rating,
#                 d.num_ratings,
#                 COUNT(*) OVER (PARTITION BY d.general_category, d.specific_category) AS application_count,
#                 COUNT(*) OVER (PARTITION BY d.general_category) AS category_count
#             FROM 
...
#                 d.general_category,
#                 d.specific_category,
#                 COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM d.date_created) = EXTRACT(YEAR FROM CURRENT_DATE) THEN d.dish_id END) AS current_year,
#                 COUNT(DISTINCT CASE WHEN EXTRACT(YEAR FROM d.date_created) = EXTRACT(YEAR FROM CURRENT_DATE) - 1 THEN d.dish_id END) AS previous_year
#             FROM 
#                 flavorlens.main.dishes d
#             JOIN 
#                 flavorlens.main.dish_ingredients di ON d.dish_id = di.dish_id
...
#                 {category_filter}
#             GROUP BY
#                 d.general_category, d.specific_category
#         ),
...
#                 COUNT(DISTINCT ia.dish_id) AS dish_count,
...
#             CASE
#                 WHEN COALESCE(yc.previous_year, 0) = 0 THEN 100
#                 ELSE ROUND((COALESCE(yc.current_year, 0) - COALESCE(yc.previous_year, 0)) * 100.0 / 
#                           NULLIF(COALESCE(yc.previous_year, 0), 1), 1)
#             END AS growth,
#             ROUND((
#                 (am.penetration * 0.7) + 
...
#             ), 1) AS appeal_score,
#             CASE
#                 WHEN am.penetration < 5 AND COALESCE(yc.current_year, 0) > COALESCE(yc.previous_year, 0) * 2 THEN 'Emerging'
#                 WHEN am.penetration < 15 AND COALESCE(yc.current_year, 0) > COALESCE(yc.previous_year, 0) * 1.5 THEN 'Trending'
#                 WHEN am.penetration > 30 THEN 'Established'
#                 WHEN am.penetration > 15 THEN 'Growing'
#                 ELSE 'Emerging'
#             END AS trend
#         FROM 
...
#                     appealScore=float(row["appeal_score"]),
#                     penetration=float(row["penetration"]),
#                     growth=float(row["growth"]),
#                     ratings=float(row["avg_rating"]) if row["avg_rating"] else 0.0,
#                     trend=row["trend"]
#                 ))
